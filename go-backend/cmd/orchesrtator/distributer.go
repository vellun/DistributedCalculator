...
func DistributeTask(exp string) error {
	postfix_slice, exp_id, err := parser.ParseExpression(exp)
	if err != nil {
		fmt.Println(err)
		return err
	}
...
func GetTask(postfix_slice []string) (int, int, int, *database.Task) {
	cnt++
	var (
		l int = 0
		m int = 1
	...
	for i := 0; i < len(postfix_slice); i++ {
		...
		if p == "+" || p == "-" || p == "*" || p == "/" { // Если правый указатель попал на оператора
			if err1 == nil && err2 == nil { // Если два токена перед оператором - числа
				task := &database.Task{Id: cnt, Operand1: postfix_slice[l],
				...
				task := &database.Task{Id: cnt, Task_id1: token,
					Operand2: postfix_slice[m]}
				return l, m, r, task
				...
				task := &database.Task{Id: cnt, Task_id1: token1,
					Task_id2: token2}
				return l, m, r, task
